...
func (self *{{.Type}}) ToBase58() []byte {
	out, err := base58.EncodeMV(&prefix.{{.Prefix}}, self[:])
	if err != nil {
		panic(err)
	}
	return out
...
func (self {{.Type}}) MarshalText() ([]byte, error) {
	return base58.EncodeMV(&prefix.{{.Prefix}}, self[:])
}
...
func (self *{{.Type}}) UnmarshalText(src []byte) error {
	...
	if pre != &prefix.{{.Prefix}} {
		return fmt.Errorf("gomav: invalid {{.Type}} encoding")
	}
	copy(self[:], payload)
	return nil
}
